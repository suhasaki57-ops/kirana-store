  ...
  return (
    <div className="rounded-xl border bg-white p-5 shadow-sm">
      ...
      <div className="space-y-3">
        {recentList.length === 0 ? (
          <p className="text-sm text-muted-foreground py-4 text-center">No orders placed yet.</p>
        ) : (
          recentList.map((o) => (
            <div key={o.id} className="flex items-center justify-between gap-3 border-b pb-3 last:border-0 last:pb-0">
              <div>
                <p className="text-sm font-semibold">{o.id}</p>
                <p className="text-xs text-muted-foreground">{o.customer} · {o.method || 'COD'}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold text-green-700">{formatPrice(o.total)}</p>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusCls[o.status] ?? 'bg-gray-100 text-gray-700'}`}>
                  {o.status}
                </span>
              </div>
            </div>
